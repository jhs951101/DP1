...
public class GrimPanFrameMain extends JFrame implements Observer{
	...
	public GrimPanFrameMain() {
		...
		rdbtnmntmRect = new JRadioButtonMenuItem("사각형");
	...
	public void measurementChanged(){
		...
		if (editState == Util.EDIT_MOVE){
			update(Util.SHAPE_NAME[model.getEditState()], "이동", model.shapeList.size()+tilemodel.tileRectList.size());
		}
		else if(editState == Util.EDIT_DELETE){
			update(Util.SHAPE_NAME[model.getEditState()], "제거", model.shapeList.size()+tilemodel.tileRectList.size());
		}
		else if(editState == Util.RECT_MOVE){
			update(Util.SHAPE_NAME[model.getEditState()], "이동-사각형", model.shapeList.size()+tilemodel.tileRectList.size());
		}
		else if(editState == Util.RECT_RESIZE){
			update(Util.SHAPE_NAME[model.getEditState()], "조절-사각형", model.shapeList.size()+tilemodel.tileRectList.size());
		}
		else {
			update(Util.SHAPE_NAME[model.getEditState()], "추가", model.shapeList.size()+tilemodel.tileRectList.size());
		}
	}
